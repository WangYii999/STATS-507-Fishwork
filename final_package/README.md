...
### One‑Click
- Windows: run `final_package\run_all.bat`
- Cross‑platform: `make all` or run `make eval2`, `make eval4`, `make images`

## Outputs
- Summary: `outputs/stage3_summary_<RUN_ID>.json`
- Cases: `outputs/stage3_cases_<RUN_ID>.jsonl`
- Done flag: `outputs/stage3_done_<RUN_ID>.txt`
...
```
Figures are written to `report_images/`: beam comparison, baseline comparison, training curves, etc.
...
- If packages are missing (e.g., `torch`, `sentencepiece`, `evaluate`), run `pip install -r final_package/requirements.txt`.
- For version control, avoid committing large weights/logs unless using Git LFS.
