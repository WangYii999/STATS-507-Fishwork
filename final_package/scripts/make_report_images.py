...
ASSETS_DIR = Path('final_package/report_images')
...
def plot_t5_compare():
    p2 = _find_latest(['final_package/outputs/stage3_summary_beams2*.json'])
    p4 = _find_latest(['final_package/outputs/stage3_summary_beams4pp*.json'])
    d2 = read_json(p2) or {}
    d4 = read_json(p4) or {}
    m2 = d2.get('t5_small', {})
    m4 = d4.get('t5_small', {})
...
def plot_t5_beam_triple():
    p1 = _find_latest(['final_package/outputs/stage3_summary_beams1*.json'])
    p2 = _find_latest(['final_package/outputs/stage3_summary_beams2*.json'])
    p4 = _find_latest(['final_package/outputs/stage3_summary_beams4pp*.json'])
    if not (p2 and p4):
        return None
...
def plot_len_compare():
    pl = _find_latest(['final_package/outputs/stage3_summary_longmax*.json'])
    pm = _find_latest(['final_package/outputs/stage3_summary_max96*.json'])
    if not (pl and pm):
        return None
    dl = read_json(pl) or {}
    dm = read_json(pm) or {}
...
def plot_baselines():
    b = read_json('final_package/outputs/baseline.json') or {}
    d_long = read_json(_find_latest(['final_package/outputs/stage3_summary_longmax*.json'])) or {}
    ret = b.get('retrieval', {})
    tmp = b.get('template', {})
...
def plot_training_curve():
    latest = _find_latest(['final_package/outputs/**/trainer_state.json'])
...
def copy_existing_images():
    ...
    for fn in ['model_arch.png','model_arch_cn.png','eval_summary.png']:
        src = Path('final_package/outputs')/fn
        if src.exists():
            dst = ASSETS_DIR/src.name
            shutil.copy2(src, dst)
            copied.append(str(dst))
    figs_dir = Path('notebooks/figs')
    for fn in ['persona_top_tokens.png','conv_len.png']:
        src = figs_dir/fn
        if src.exists():
            dst = ASSETS_DIR/src.name
            shutil.copy2(src, dst)
